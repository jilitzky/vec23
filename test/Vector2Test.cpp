...
namespace Vec23::Test
{
    ...
    TEST(Vector2Test, SlerpBasic)
    {
        FVector2 start(1.0f, 0.0f);
        FVector2 end(0.0f, 1.0f);
        FVector2 mid = FVector2::Slerp(start, end, 0.5f);
        EXPECT_TRUE(mid.IsNearlyEqual({ 0.70710f, 0.70710f }));
        EXPECT_TRUE(mid.IsNormalized());
    }

    TEST(Vector2Test, SlerpCollinear)
    {
        FVector2 start(1.0f, 0.0f);
        FVector2 end(2.0f, 0.0f);
        FVector2 mid = FVector2::Slerp(start, end, 0.5f);
        EXPECT_FALSE(std::isnan(mid.x));
        EXPECT_TRUE(mid.IsNearlyEqual({ 1.5f, 0.0f }));
    }

    TEST(Vector2Test, SlerpDifferentMagnitudes)
    {
        FVector2 start(10.0f, 0.0f);
        FVector2 end(0.0f, 20.0f);
        FVector2 mid = FVector2::Slerp(start, end, 0.5f);
        EXPECT_TRUE(mid.GetNormalized().IsNearlyEqual({ 0.70710f, 0.70710f }));
