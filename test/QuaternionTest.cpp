...
namespace Vec23::Test
{
    ...
    TEST(QuaternionTest, Usage)
    {
        // Compute the dot product of two 2D vectors.
        Vector2<float> a(1.0f, 0.0f);
        Vector2<float> b(2.0f, 0.0f);
        float dot = a.Dot(b); // dot is 2.0f
        EXPECT_NEAR(dot, 2.0f, kToleranceEpsilon<float>);

        // Calculate the angle between two 3D vectors.
        Vector3<float> p1(1.0f, 0.0f, 0.0f);
        Vector3<float> p2(0.0f, -1.0f, 0.0f);
        float angle = FVector3::Angle(p1, p2); // angle is 90.0f
        EXPECT_NEAR(angle, 90.0f, kToleranceEpsilon<float>);
